...
1. 将 $M$ 个元素（可区分）分为 $n$ 个组，每组大小为 $m_1, m_2, \dots, m_n$（可有重复）。设共有 $k$ 种不同的组大小，其中大小为 $s_j$ 的组出现 $r_j$ 次（$j = 1,2,\dots,k$），则：

| 类型术语 | 场景特征       | 通用公式                                                                            | 示例（6人分4,1,1）                                                                                  |
| :------- | :------------- | :---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 定组定额 | 组有别，数固定 | $\displaystyle \frac{M!}{\prod _{i=1}^n m_i!}$                                      | $\displaystyle \frac{6!}{4! \times 1! \times 1!}$<br>（甲4乙1丙1）                                  |
| 完全分堆 | 组无别，纯分堆 | $\displaystyle \frac{M!}{\prod _{i=1}^n m_i!} \cdot \frac{1}{\prod _{j=1}^k r_j!}$  | $\displaystyle \frac{6!}{4! \times 1! \times 1!} \cdot \frac{1}{2!}$<br>（分三堆4，1，1）           |
| 完全分配 | 先分堆，后排列 | $\displaystyle \frac{M!}{\prod _{i=1}^n m_i!} \cdot \frac{n!}{\prod _{j=1}^k r_j!}$ | $\displaystyle \frac{6!}{4! \times 1! \times 1!} \cdot \frac{3!}{2!}$<br>（甲乙丙从三堆中任取一堆） |
...
2. 插空法（解决“不相邻”问题）：首先排列可以相邻的B类元素，然后B类元素之间和两端生成“空”，再将不能相邻的A类元素插入这些“空”中

> [!NOTE]
> 周一到周日七天中选出不相邻的三天
> A类元素3天，B类元素4天，形成5个空，5个空中选出3天，共有 $C_5^3$ 种

3. 隔板法： $n$ 个相同物体放入 $k$ 个不同盒子
...
5. 错排数：将 $n$ 个元素重新排列，所有元素都没有出现在原来位置上
...
4. 空瓶换酒： $M$ 个空瓶可以换 $N$ 瓶酒，现有 $x$ 个空瓶，在**不赊账**的前提下能换多少瓶酒？其中 $M$ 、 $N$ 、 $x$ 都是正整数，且 $x \geq M > N$
...
5. 钟表
    ...
    + 半天（12h），时针与分针
      + 重合11次（排除12:00这个特殊时刻）
      + 垂直22次（只有2:00-3:00和8:00到9:00垂直1次，其余每1个小时内均垂直2次）

6. 天平称币问题：在使用天平称重的情况下，若称 $n$ 次，最多可以从 $3^n$ 张纸币中辨别出那一张是假币

7. 库存比例 $S_1:S_2:\cdots:S_n$ ，消耗比例 $C_1:C_2:\cdots:C_n$ ，用完时间为 $\displaystyle t_i = \frac{S_i}{C_i}$ ，时间最小最先用完